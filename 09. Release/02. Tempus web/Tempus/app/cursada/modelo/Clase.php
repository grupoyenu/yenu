...
class Clase
{
    ...
    public function borrar(): array
    {
        ...
        Log::guardar("INF", "CLASE --> BORRAR ID INVALIDO");
    ...
    public function crear(): array
    {
        if ($this->diaSemana && $this->plan && $this->horaInicio && $this->horaFin && $this->aula) {
            ...
            $consulta = "INSERT INTO clase (id, idAula, idPlan, diaSemana,horaInicio, horaFin, fechaCreacion, fechaEdicion) VALUES (NULL, {$idAula}, {$this->plan}, {$this->diaSemana}, '{$this->horaInicio}', '{$this->horaFin}', NOW(), NULL)";
            $resultado = Conexion::getInstancia()->insertar($consulta);
            $this->id = ($resultado[0] == 2) ? $resultado[2] : NULL;
            return $resultado;
        }
        Log::guardar("INF", "CLASE --> CREAR DIA, PLAN, HORA INICIO, HORA FIN O AULA INVALIDA");
    ...
    private function verificarDisponibilidad($idAula): array
    {
        ...
        Log::guardar("INF", "CLASE --> VERIFICAR DISPONIBILIDAD AULA INVALIDA");
        return array(0, "No se pudo hacer referencia al aula de clase");
    }

    public function modificar(): array
    {
        if ($this->id && $this->horaInicio && $this->horaFin && $this->aula) {
            $idAula = $this->aula->getId();
            $consulta = "UPDATE clase SET "
                . " horaInicio = '{$this->horaInicio}',"
                . " horaFin = '{$this->horaFin}', "
                . " idAula = {$idAula}, "
                . " fechaEdicion = NOW() WHERE id = {$this->id}";
            return Conexion::getInstancia()->modificar($consulta);
        }
        Log::guardar("INF", "CLASE --> MODIFICAR ID, HORA INICIO, HORA FIN O AULA INVALIDA");
    ...
    {
        if ($this->id) {
            $consulta = "SELECT * FROM clase WHERE id = {$this->id}";
            $resultado = Conexion::getInstancia()->obtener($consulta);
        ...
        Log::guardar("INF", "CLASE --> OBTENER POR IDENTIFICADOR INVALIDO");
        return array(0, "No se pudo hacer referencia a la clase");
    }
