...
class ColeccionClases
{
    ...
    public static function buscarParaInforme($carrera, $asignatura, $dia, $modificada, $opDesde, $desde, $opHasta, $hasta)
    {
        $consulta = "SELECT * FROM vw_cursada WHERE "
        ...
        $consulta .= ($desde != "NO") ? " (horaInicioLunes {$opDesde} '{$desde}' OR horaInicioMartes {$opDesde} '{$desde}' OR horaInicioMiercoles {$opDesde} '{$desde}' OR horaInicioJueves {$opDesde} '{$desde}' OR horaInicioViernes {$opDesde} '{$desde}' OR horaInicioSabado {$opDesde} '{$desde}') AND " : "";
        $consulta .= ($hasta != "NO") ? " (horaFinLunes {$opHasta} '{$hasta}' OR horaFinMartes {$opHasta} '{$hasta}' OR horaFinMiercoles {$opHasta} '{$hasta}' OR horaFinJueves {$opHasta} '{$hasta}' OR horaFinViernes {$opHasta} '{$hasta}' OR horaFinSabado {$opHasta} '{$hasta}') AND " : "";
